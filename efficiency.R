...
effi_cal1<-function(nn,designmatrix,opt.type="T",model=1,effect=1,flag_circular=0,sigma=diag(nrow(designmatrix))){
  p=nrow(designmatrix)
  n=ncol(designmatrix)
  t=max(as.vector(designmatrix))
  design.base=find_good_sequence1(p,t,model = model,effect = effect,flag_circular = flag_circular,sigma=sigma)
  ...
  if(model==1){
    ...
    for(i in 1:n){
      Clist=s.Cij1(designmatrix[,i],t,p,model=model,effect=effect,flag_circular = flag_circular,sigma=sigma)
