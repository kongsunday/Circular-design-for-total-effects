...
find_good_design1<-function(n=20,p=4,t=3,model=1,effect=1,flag_circular=0,sigma=diag(p)){
  ### the searching process
# covariance matrix
  bt=diag(1,t)-matrix(1/t,t,t)
  design.base=find_good_sequence1(p,t,model=model,effect=effect,flag_circular = flag_circular,sigma = sigma)##more general function?
  ...
  num.mat=length(s.Cij1(full.Tau[,1],t,p,model = model,effect = effect,flag_circular = flag_circular,sigma = sigma))-3
  if(num.mat==3){
    all.matrix=apply(full.Tau,2,function(x){temp=s.Cij(x,t,p,model = model,effect = effect,flag_circular = flag_circular,sigma = sigma);
  ...
  if(num.mat==6){
    all.matrix=apply(full.Tau,2,function(x){temp=s.Cij1(x,t,p,model = model,effect = effect,flag_circular = flag_circular,sigma = sigma);
    ...
    param=list(timeLimit = 500)
    result=gurobi(test,param)
  }
  index=as.integer(result$x[1:(num.col*prod(1:t))]+0.1)
  final.design=full.Tau[,rep((1:length(index)),index)]
  ef=effi_cal1(n,final.design,opt.type = "ALL",model=model,effect=effect,flag_circular = flag_circular,sigma = sigma)
  print(num.mat)
